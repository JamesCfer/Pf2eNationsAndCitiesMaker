...
export function sanitizeSettlement(raw, formData = {}) {
  ...
  const out = {
    ...
    stats: {
      ...
      occupiedBy: (typeof stats.occupiedBy === 'string' && stats.occupiedBy) ? stats.occupiedBy : null,
    },
    treasury: {
    ...
    stores: Array.isArray(s.stores) ? s.stores.map(st => ({
      ...
      jobs: Array.isArray(st?.jobs) ? st.jobs.map(j => ({
        id:              safeString(j?.id, `job-${shortId()}`),
        title:           safeString(j?.title, 'Untitled Job'),
        description:     safeString(j?.description, ''),
        reward:          safeNum(j?.reward, 0, 0, 9_999_999),
        status:          ['open', 'claimed', 'completed', 'failed'].includes(j?.status) ? j.status : 'open',
        assignedActorId: j?.assignedActorId || null,
      })) : [],
    })) : [],
